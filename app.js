...
function migrateOldData() {
  // Fix: rename any "Example Ranch" org to "Example Org"
  const data = getData();
  if (data.orgs['Example Ranch']) {
    if (!data.orgs['Example Org']) data.orgs['Example Org'] = { ranches: {} };
    // Merge ranches into Example Org
    for (const r of Object.keys(data.orgs['Example Ranch'].ranches)) {
      if (!data.orgs['Example Org'].ranches[r]) {
        data.orgs['Example Org'].ranches[r] = data.orgs['Example Ranch'].ranches[r];
      }
    }
    delete data.orgs['Example Ranch'];
    saveData(data);
  }
  ...
  if (oldProjects) {
    try {
      ...
      // All old projects migrate under "Example Org" org, grouped by ranch column
  ...
  if (oldProgress) {
    ...
    if (!data.orgs['Example Org']) data.orgs['Example Org'] = { ranches: {} };
    if (!data.orgs['Example Org'].ranches['EXAMCODE(EXAMPLE RANCH)']) {
      data.orgs['Example Org'].ranches['EXAMCODE(EXAMPLE RANCH)'] = { rows: oldRows, progress: {} };
    }
