...
function parseCodeName(s) {
  ...
  // Closing paren is OPTIONAL — handles common typos like "EXAMPLE CITRUS VENTURES (EXAMCIT".
  const m = str.match(/^([^()]+?)\s*\(\s*([^()]+?)\s*\)?\s*$/);
  if (!m) {
    // No parens — treat the entire string as the NAME. PickTrace exports many
    // sites as plain names (e.g. "CENTRAL", "DESERT", "JOHN SMITH") with no
...
function parseSitesMaster(headers, rows, fileName) {
  ...
  // when the master CSV stores Site like "EXAMCIT(...)" or "EXAMPLE... (EXAMCIT".
...
// Templates list sites in formats like "EXAMPLE FARMS INC. (EXAMFAR)" or "JOHN SMITH" or "3EXCITR (3 EXAMPLE CITRUS, LLC.)".
...
//   - Strip ALL commas        ("ACME FARMING, INC."  → "ACME FARMING INC.")
//   - Strip TRAILING periods  ("ACME FARMING INC."   → "ACME FARMING INC")
//   - Internal periods preserved (initials like "J. SMITH" stay intact)
...
// "J. SMITH FARMS, INC." vs "J. SMITH FARMS, INC" reconcile.
...
function computeSitesToCreate(newAdds, toUnarchive) {
  ...
  buckets.forEach(({ entry: a, kind }) => {
    ...
    //   2. Embedded grower code match (handles CSV typos like "EXAMPLE RANCH... (EXAMRAN")
    //   3. Word-prefix match — handles "JOHN SMITH FAMILY TRUST" vs CSV's "JOHN SMITH"
...
function exportXlsx() {
  ...
  //   Site: "EXAMPUB(EXAMPLE PROPERTIES, INC.)"
  //   Name: "10001(BLK#1 EXAMPLE)"
